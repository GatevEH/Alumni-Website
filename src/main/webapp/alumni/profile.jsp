...
<sql:setDataSource var = "snapshot" driver = "com.mysql.jdbc.Driver" 
    ...
    <sql:query dataSource = "${snapshot}" var = "resultAlumni">
		select * from alumnis, specialties where fk_specialty = specialty_id and alumni_id = <%=request.getSession().getAttribute("userID")%>
    </sql:query>
    
<div class="pageContent container-fluid" style="padding: 20px 20px;">
	<div  class="row" style="padding: 20px 20px; min-height:475px; width: 100%;">
    	<div class="sideMenu col-md-3" style="width: 230px;">
    		<a href="jobMyJobs.jsp" class="sideButton">My Job Offers</a>		
    		<a href="<%=request.getContextPath()%>/Logout" class="sideButton">Logout</a>
    	</div>
    	
    		<c:forEach items="${resultAlumni.rows}" var="row">
	    	<div class="col-4" style="margin-left: 80px;">
					<h1 style="font-weight: bold;">${row.first_name} ${row.last_name}</h1>
			    	<img src="data:profile_picture/jpg;base64, <%=ad.getAlumniPhoto(Integer.parseInt(request.getSession().getAttribute("userID").toString()))%>" width="250" height="280"/>	
			 </div>
			 <div class="col-5">
